...
import Maze.Utils exposing (AlgorithmExtra(..), Direction(..), Height, Index, Maze, MazeCommon, Width, deleteEdge, getRandomIndex, getRandomValue, moveIndexDown, moveIndexLeft, moveIndexRight, moveIndexUp)
...
getFrontier prevFrontier visitedIndexes index width height =
    let
        nothingIfListContains value list =  
            value |> 
                Maybe.andThen (\num -> if List.member num list then Nothing else Just num) 
        ...
        frontier =
            [ indexUp, indexDown, indexRight, indexLeft ]
